UPDATE propiedades SET 
nombre = 'Casa Vacacional Monterrico - Paraíso en la Playa',
descripcion = 'Hermosa casa vacacional en Monterrico con todas las comodidades para una estadía inolvidable.

HABITACIONES Y ESPACIOS:
• 3 habitaciones amplias y confortables
• Habitación principal con aire acondicionado
• 2 habitaciones secundarias espaciosas
• Capacidad para 8-10 personas
• Sala, comedor y cocina en concepto abierto
• Áreas verdes y jardín amplio

ÁREAS RECREATIVAS:
• Piscina privada
• Pérgola con área de descanso
• Churrasquera/BBQ para asados familiares
• Estacionamiento privado techado

BAÑOS:
• 3 baños completos
• Baño privado en habitación principal
• Baño junto a la piscina
• Ducha exterior con agua caliente

COCINA Y SERVICIOS:
• Cocina completamente equipada
• Refrigerador, estufa y horno
• Utensilios de cocina completos
• Área de comedor amplia
• WiFi de alta velocidad
• Aire acondicionado en habitaciones principales

EXTERIORES:
• Jardín tropical con árboles frutales
• Áreas verdes para recreación
• Espacio amplio para parqueo múltiple
• Ambiente tropical y relajante
• Terraza y áreas de descanso

Ideal para familias, grupos de amigos o reuniones especiales. ¡Tu escape perfecto en la playa de Monterrico!'
WHERE id = 1;
